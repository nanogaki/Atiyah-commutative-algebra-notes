...
\begin{exercise}\label{ex5.1}
	$f^*(V(I))=\left\{f^{-1}(\mathfrak{q}):\mathfrak{q}\supseteq I\right\}$. Use \Cref{5.10} to prove $f^*(V(I))\supseteq V\left(f^{-1}(I)\right)$.
\end{exercise}
...
\begin{exercise}\label{ex5.5}
	\begin{enumerate}
		\item[(ii)] Use \Cref{5.10} to prove that the contraction of maximal ideals in $B$ are maximal in $A$.
	\end{enumerate}
\end{exercise}
...
\begin{exercise}\label{ex5.8}
	Prove that there is a ring $D\supseteq B$ such that $f$ and $g$ factor into linear factors in $D$. Use Kronecker's construction.
\end{exercise}
...
\begin{exercise}\label{ex5.15}
	If $L/K$ is separable, we can find a finite Galois extension $M/K$ such that $L\subseteq M$. Let $C$ be the integral closure of $A$ in $M$. By \Cref{ex5.14} and \Cref{ex5.13}, $P_C$ is finite. To prove $P_B=\left\{\mathfrak{p}\cap B:\mathfrak{p}\in P_C\right\}$, we need \Cref{5.10}, so $\mathfrak{p}_B\in P_B$ can be lifted to $\mathfrak{p}_C$. Verify $\mathfrak{p}_C\in P_C$.
	
	If $L/K$ purely inseparable, fix a $\mathfrak{p}\subseteq A$ and pick $\mathfrak{q}\subseteq B$ such that $\mathfrak{q}\cap A=\mathfrak{p}$. Let $x\in\mathfrak{q}$, so $x^{p^m}\in K$. $x$ is integral over $A$, so is $x^{p^m}$. But $A$ integrally closed, so $x^{p^m}\in A$. Therefore, $x^{p^m}\in\mathfrak{q}\cap A=\mathfrak{p}$. If there is another $\mathfrak{q}^\prime$ whose contraction is $\mathfrak{p}$, then $x^{p^m}\in\mathfrak{q}^\prime$ and so $x\in\mathfrak{q}^\prime$. Thus, such picked $\mathfrak{q}$ is unique.
\end{exercise}
...
\begin{exercise}\label{ex5.19}
	By \Cref{ex5.18}, $k\left[t_1,\cdots,t_n\right]/\mathfrak{m}$ is a finite algebraic extension of $k$, so $k\left[t_1,\cdots,t_n\right]/\mathfrak{m}\cong k$. Let $t_i\mapsto a_i$, then $t_i-a_i\in\mathfrak{m}$. Prove that $\left(t_1-a_1,\cdots,t_n-a_n\right)$ is maximal.
\end{exercise}
...
\begin{exercise}\label{ex5.27}
	\begin{enumerate}
		\item[$\Rightarrow$:] Use \Cref{5.21} to prove maximal elements are valuation rings.
...
\begin{exercise}\label{ex5.32}
	$v(x)+v(y)=v(xy)$ implies $v(A-\mathfrak{p})$ is closed under addition. Let $\Delta=v(A-\mathfrak{p})\cup\left(-v(A-\mathfrak{p})\right)$. If $0\leqslant v(x)\leqslant v(y)$ with $y\in A-\mathfrak{p}$, then $x^{-1}y\in A$ and $x\left(x^{-1}y\right)\in A-\mathfrak{p}$. Since $A-\mathfrak{p}$ is saturated as a multiplicatively closed set, $x\in A-\mathfrak{p}$.
	
	Let $S=\left\{x\in A:v(x)\in\Delta,v(x)\geqslant0\right\}$. If $x,y\notin S$, $v(x+y)\geqslant\min\left(v(x),v(y)\right)$ means $x+y\notin S$; if $x\notin S$ and $y\in A$, $v(xy)\geqslant v(x)$ means $xy\notin S$. Hence, $A-S$ is an ideal, and it is automatically prime due to $S$ is multiplicatively closed.
	
	Define $v_{A_\mathfrak{p}}:K^*\rightarrow\Gamma/\Delta$ to be $x\mapsto x+\Delta$, then $A_\mathfrak{p}=\left\{x\in K:v_{A_\mathfrak{p}}(x)\geqslant0\right\}$.
	
	The fraction field of $A/\mathfrak{p}$ is not $K$ but $\left(A/\mathfrak{p}\right)_{\overline{\mathfrak{p}}}$. Let $x=\overline{a}/\overline{b}\in\left(A/\mathfrak{p}\right)_{\overline{\mathfrak{p}}}$ and define $v_{A/\mathfrak{p}}(x)=v(a)-v(b)$, so $v_{A/\mathfrak{p}}(x)\in\Delta$. We have $v_{A/\mathfrak{p}}(A/\mathfrak{p})=\Delta_+$.
\end{exercise}

\begin{exercise}\label{ex5.33}
	Let $v(a/b)=v_0(a)-v_0(b)$. Use $v(xy)=v(x)+v(y)$ to prove it is well-defined and unique.
\end{exercise}

\begin{exercise}\label{ex5.34}
	Since $gf(A)=A$, $g(B)\supseteq A$. Let $\mathfrak{n}$ be a maximal ideal of $g(B)$, by \Cref{ex5.10}, $\mathfrak{m}=\mathfrak{n}\cap A$ is the maximal ideal of $A$. Also since $\left(g(B)\right)_\mathfrak{n}$ dominates $A$, by \Cref{ex5.27}, $\left(g(B)\right)_\mathfrak{n}=A$ and therefore $g(B)\subseteq A$.
\end{exercise}

\begin{exercise}\label{ex5.35}
	Let $A^\prime$ be an arbitrary valuation ring of $\operatorname{Frac}(B)$ containing $f(A)$. By hypothesis, $\operatorname{Spec}\left(B\otimes_AA^\prime\right)\rightarrow\operatorname{Spec}\left(A^\prime\right)$ is a closed map, and by \Cref{ex5.34}, $g:B\otimes_AA^\prime\rightarrow\operatorname{Frac}(B)$ defined by $b\otimes a^\prime\mapsto ba^\prime$ satisfies $g\left(B\otimes_AA^\prime\right)=A^\prime$. In other words, $ba^\prime\in A^\prime$ for all $b\in B$ and all $a^\prime\in A^\prime$. By taking $a^\prime=1$, we have $B\subseteq A^\prime$. By \Cref{5.22}, $B$ is in the integral closure of $f(A)$, so $B$ is integral over $f(A)$.
	
	The composition map $A\rightarrow B\rightarrow B/\mathfrak{p}_i$ is integral. By \Cref{ex5.6}, $A\rightarrow\prod_{i=1}^nB/\mathfrak{p}_i$ is integral, Using integral dependence equation can prove $A\rightarrow A/\mathfrak{R}$ is integral then $A\rightarrow B$ is integral.
\end{exercise}
